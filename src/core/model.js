...
export const clubById = (id) => CLUBS.find((c) => c.id === id || String(c.id) === String(id));
export const personById = (id) => PEOPLE.find((p) => p.id === id || String(p.id) === String(id));
export const registerRealClubs = (realClubs = []) => {
    realClubs.forEach((rc) => {
        const id = String(rc.id);
        const existing = CLUBS.find((c) => String(c.id) === id);
        if (!existing) {
            CLUBS.push({
                id,
                name: rc.name,
                fullName: rc.fullName || rc.name,
                category: rc.category || 'Khác',
                mark: (rc.code || rc.name || 'CLB').slice(0, 4).toUpperCase(),
                color: rc.color || '#c95628',
                tone: rc.tone || 'code',
                tagline: rc.tagline || rc.description || 'Cộng đồng FPTU',
                description: rc.description || 'Câu lạc bộ sinh viên FPTU.',
                schedule: rc.scheduleLabel || 'Đang cập nhật',
                place: rc.locationLabel || 'Khuôn viên FPTU',
                recruiting: rc.isRecruiting ?? true,
                finance: false,
                founded: rc.founded || 2026,
                tags: rc.tags || [rc.category || 'Cộng đồng'],
            });
        }
    });
};
export const registerRealPerson = (user) => {
    if (!user) return;
    const id = String(user.id || user.email);
    const existing = PEOPLE.find((p) => String(p.id) === id);
    if (!existing) {
        PEOPLE.push({
            id,
            name: user.name || user.fullName || 'Sinh viên FPTU',
            initials: (user.name || user.fullName || 'SV')
                .split(' ')
                .slice(-2)
                .map((n) => n[0])
                .join('')
            ...
            headline: 'Sinh viên FPT University',
            major: 'Kỹ thuật phần mềm',
            code: user.studentCode || 'SE180000',
            year: 'K18',
            interest: 'Công nghệ · Hoạt động · Trải nghiệm',
            about: 'Thành viên FPTU ClubHub.',
            skills: ['Học hỏi', 'Đồng đội'],
            project: '',
            label: `${user.name || 'Bạn'} · Tài khoản hiện tại`,
        });
    }
};
...
export const resetRealEntities = () => {
    PEOPLE.splice(INITIAL_PEOPLE_COUNT);
};

// One mutation boundary validates actor + club + resource relationship, even in mock mode.
export function transition(state, action) {
    let type = action.type;
    if (type === 'submitQuest') type = 'contribute';
    if (type === 'reviewSubmission') type = 'reviewContribution';
    if (type === 'updateProfile') type = 'profile';

    const userId = action.userId || action.actorId;
    const { clubId, term = CURRENT_TERM } = action;
    const payload = action.payload || action;
    const fail = (message) => ({ state, error: message });

    if (!personById(userId)) return fail('Nhân vật không hợp lệ hoặc chưa xác định.');
    if (type === 'profile') {
        const profiles = {
            ...state.profiles,
            [userId]: {
        ...
        return { state: { ...state, profiles }, message: 'Đã cập nhật hồ sơ cá nhân.' };
    }
    ...
    let message = 'Đã cập nhật dữ liệu.';
    switch (type) {
        case 'apply': {
