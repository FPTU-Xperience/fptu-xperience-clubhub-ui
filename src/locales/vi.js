...
const ERROR_MESSAGE_TRANSLATIONS = Object.freeze({
    ...
    'the approved report does not contain valid event information.':
        'Báo cáo đã duyệt không chứa thông tin sự kiện hợp lệ.',
    ...
    'one or more members do not belong to this club or joined after the activity started.':
        'Một hoặc nhiều thành viên không thuộc câu lạc bộ này hoặc tham gia sau khi hoạt động bắt đầu.',
    'username, full name, email and password are required.':
        'Tên đăng nhập, họ tên, thư điện tử và mật khẩu là bắt buộc.',
    'username, full name or email exceeds the allowed length.':
        'Tên đăng nhập, họ tên hoặc thư điện tử vượt quá độ dài cho phép.',
    ...
    'each account must have exactly one predefined actor role.':
        'Mỗi tài khoản phải có đúng một vai trò được định nghĩa trước.',
    ...
    'you cannot deactivate your own account or change your own actor role.':
        'Bạn không thể vô hiệu hóa tài khoản hoặc thay đổi vai trò của chính mình.',
    ...
    'only predefined FPTU-Xperience actor roles are supported.':
        'Hệ thống chỉ hỗ trợ các vai trò FPTU-Xperience đã được định nghĩa.',
    ...
    'the final active admin account cannot be deactivated or reassigned.':
        'Không thể vô hiệu hóa hoặc đổi vai trò của tài khoản quản trị viên hoạt động cuối cùng.',
    ...
    'a proposal can link either an existing activity or a future event report, not both.':
        'Đề xuất chỉ được liên kết với một hoạt động hoặc một báo cáo sự kiện sắp tới.',
    'the selected future event report does not belong to this club.':
        'Báo cáo sự kiện đã chọn không thuộc câu lạc bộ này.',
    'only a future event report awaiting its budget can be linked.':
        'Chỉ có thể liên kết báo cáo sự kiện sắp tới đang chờ lập ngân sách.',
    ...
    'weekly, monthly, or attendance activities cannot be linked to a budget proposal.':
        'Không thể liên kết hoạt động hằng tuần, hằng tháng hoặc điểm danh với đề xuất ngân sách.',
    'a cancelled activity cannot be linked to a budget proposal.':
        'Không thể liên kết hoạt động đã hủy với đề xuất ngân sách.',
    'proposal title and description are required.': 'Tên và mô tả đề xuất là bắt buộc.',
    'review this budget together with its future event report.':
        'Vui lòng xét duyệt ngân sách cùng với báo cáo sự kiện sắp tới liên quan.',
    'only proposals awaiting club owner review can be approved.':
        'Chỉ có thể duyệt đề xuất đang chờ chủ nhiệm xét duyệt.',
    'only proposals awaiting club owner review can be rejected.':
        'Chỉ có thể từ chối đề xuất đang chờ chủ nhiệm xét duyệt.',
    'the proposal creator cannot approve their own proposal.':
        'Người tạo đề xuất không thể tự phê duyệt đề xuất của mình.',
    'the proposal creator cannot reject their own proposal.':
        'Người tạo đề xuất không thể tự từ chối đề xuất của mình.',
    'the club owner must approve this proposal before final approval.':
        'Chủ nhiệm phải duyệt đề xuất trước bước phê duyệt cuối.',
    'the club owner must review this proposal before final rejection.':
        'Chủ nhiệm phải xét duyệt đề xuất trước bước từ chối cuối.',
    ...
    'the proposal creator cannot approve its settlement.':
        'Người tạo đề xuất không thể tự duyệt quyết toán của đề xuất đó.',
    ...
    'treasurer must be an approved member of this club.':
        'Thủ quỹ phải là thành viên đã được duyệt của câu lạc bộ này.',
    ...
    'a pending transfer request already exists for this club':
        'Câu lạc bộ đã có một yêu cầu chuyển quyền đang chờ xử lý.',
    ...
    'attachments can only be changed on draft or rejected reports.':
        'Chỉ có thể thay đổi tệp đính kèm của báo cáo nháp hoặc báo cáo bị yêu cầu sửa.',
    ...
    'assign at least one club treasurer before submitting a future event report.':
        'Vui lòng chỉ định ít nhất một thủ quỹ trước khi nộp báo cáo sự kiện sắp tới.',
    'only future event reports can receive a linked budget.':
        'Chỉ báo cáo sự kiện sắp tới mới có thể liên kết ngân sách.',
    ...
    'the treasurer must submit the event budget before club owner review.':
        'Thủ quỹ phải nộp ngân sách sự kiện trước khi chủ nhiệm xét duyệt.',
    'only reports forwarded by the club manager can be approved.':
        'Chỉ báo cáo do chủ nhiệm chuyển tiếp mới có thể được phê duyệt.',
    'the future event package is incomplete.': 'Hồ sơ sự kiện sắp tới chưa đầy đủ.',
    'only submitted or under-review reports can be rejected.':
        'Chỉ báo cáo đã nộp hoặc đang xét duyệt mới có thể bị từ chối.',
    'reportid is required.': 'Mã báo cáo là bắt buộc.',
    'exporttype must be pdf, xlsx, or docx.': 'Định dạng xuất phải là PDF, XLSX hoặc DOCX.',
    'unable to generate the export file. the system will retry automatically.':
...
export function formatErrorMessage(error, fallback = vi.errors.server) {
    ...
    if (
        ...
        lower.includes('email/username') ||
        lower.includes('password is incorrect')
    )
        return vi.errors.credentials;
    if (lower.includes('unauthorized') || lower.includes('token expired') || lower.includes('phiên đăng nhập'))
