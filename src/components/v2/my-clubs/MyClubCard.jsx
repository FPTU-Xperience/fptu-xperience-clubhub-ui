...
export default function MyClubCard({ club }) {
    ...
    return <article className="v2-my-club-card">
        <div className="v2-my-club-mark" aria-hidden="true">{club.logoUrl ? <img src={club.logoUrl} alt="" /> : club.name.slice(0, 3).toUpperCase()}</div>
        <div className="v2-my-club-body">
            <div className="v2-my-club-top"><span className={`v2-my-club-role ${manager ? 'is-manager' : ''}`}>{manager ? 'Chủ nhiệm' : 'Thành viên'}</span></div>
            <h2>{club.name}</h2>
