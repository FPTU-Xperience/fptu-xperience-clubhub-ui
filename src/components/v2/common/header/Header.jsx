...
export default function ProductionHeader({ user, onLogout, api }) {
    ...
    useEffect(() => {
        if (!notifications.isOpen) return undefined;
        const dismissOnEscape = (event) => {
            if (event.key === 'Escape') notifications.close();
        };
        window.addEventListener('keydown', dismissOnEscape);
        return () => window.removeEventListener('keydown', dismissOnEscape);
    }, [notifications.isOpen, notifications.close]);
    ...
    return (
        <header className="v2-header">
            ...
            <div className="v2-header-actions">
                ...
                <div className="v2-header-account" aria-label={`Tài khoản hiện tại: ${displayName}`}>
                ...
                <button className="v2-header-icon-button" type="button" onClick={signOut} aria-label="Đăng xuất">
