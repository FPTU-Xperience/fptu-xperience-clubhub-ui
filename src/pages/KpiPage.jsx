...
const DEFAULT_RULES = [
    ...
    { code: 'ACTIVITY', name: 'Hoạt động', points: 5, description: 'Cho mỗi hoạt động trong báo cáo đã duyệt' },
    {
        ...
        name: 'Người tham gia',
        points: 0.1,
        description: 'Cho mỗi người tham gia hoạt động đã duyệt',
    },
    { code: 'REJECTED_REPORT', name: 'Báo cáo bị từ chối', points: -10, description: 'Cho mỗi báo cáo bị từ chối' },
    {
        ...
        points: -20,
        description: 'Cho mỗi bản nháp hoặc báo cáo bị từ chối đã quá hạn',
    },
];
...
function RatingBadge({ rating }) {
    return (
        <span
            className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-wider ${RATING_STYLES[rating] || RATING_STYLES['Needs Improvement']}`}
        >
...
function ClubKpiDetail({ club }) {
    ...
    const nextLabel =
        score >= 500
            ? 'Đã đạt mức xếp loại cao nhất'
            : `Còn ${formatPoints(nextTarget - score)} điểm để lên mức tiếp theo`;
        ...
        <div className="space-y-5">
            <section className="grid gap-5 xl:grid-cols-[1.1fr_1.9fr]">
                ...
                >
                    ...
                    <p className="relative text-xs font-bold uppercase tracking-[0.22em] text-cyan-300">
                        Điểm KPI câu lạc bộ
                    </p>
                    ...
                    <div className="relative mt-5">
                        <RatingBadge rating={club.rating} />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
                    ...
                    <MetricCard
                        icon={Target}
                        label="Điểm bị trừ"
                        value={Number(club.rejectedReports || 0) * 10 + Number(club.overdueReports || 0) * 20}
                        tone="rose"
                    />
                </div>
            </section>
...
export default function KpiPage() {
    ...
    const overview = useMemo(
        () => ({
            ...
            averageScore: clubs.length
                ? clubs.reduce((sum, club) => sum + Number(club.points || 0), 0) / clubs.length
                : 0,
    ...
    return (
        <div className="space-y-6">
            <section className="rounded-3xl bg-gradient-to-l from-orange-500 to-white-950/40 p-6 sm:p-8">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <div className="flex items-center gap-2 text-cyan-300">
                            <BarChart3 size={18} />
                            <span className="text-xs font-bold uppercase tracking-[0.24em]">Phân tích hiệu quả</span>
                        </div>
                        <h1 className="mt-3 text-3xl font-black text-white">KPI CÂU LẠC BỘ</h1>
                        <p className="mt-2 max-w-2xl text-sm text-slate-400">
                            {isAdmin
                                ? 'So sánh kết quả KPI của toàn bộ câu lạc bộ.'
                                : 'Xem kết quả KPI của câu lạc bộ mà bạn là thành viên được duyệt.'}
                        </p>
                    </div>
                    <form onSubmit={applyPeriod} className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto">
                        ...
                        <button
                            type="button"
                            onClick={loadKpis}
                            aria-label="Làm mới KPI"
                            className="flex items-center justify-center rounded-xl px-4 py-3 text-slate-300 transition hover:border-cyan-400/50 hover:text-cyan-300"
                        >
                ...
                <p className="mt-5 text-xs text-slate-600">
                    ...
                    {calculatedAt
                        ? ` · Cập nhật ${new Intl.DateTimeFormat('vi-VN', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'Asia/Ho_Chi_Minh' }).format(new Date(calculatedAt))}`
                        : ''}
                </p>
            </section>

            {error && (
                <div className="rounded-2xl border border-rose-400/30 bg-rose-400/10 p-4 text-sm text-rose-300">
                    {error}
                </div>
            )}

            {isLoading && clubs.length === 0 ? (
                <div className="flex min-h-72 items-center justify-center rounded-3xl border border-slate-800 bg-slate-900/50">
                    <div className="h-11 w-11 animate-spin rounded-full border-4 border-slate-700 border-t-cyan-400" />
                </div>
            ) : clubs.length === 0 ? (
                <div className="rounded-3xl border border-dashed border-slate-700 bg-slate-900/40 px-6 py-20 text-center">
                    ...
                    <p className="mt-2 text-sm text-slate-500">
                        {isAdmin
                            ...
                            : 'Bạn cần là thành viên được duyệt để xem KPI câu lạc bộ.'}
                    </p>
                </div>
            ) : isAdmin ? (
                <>
                    ...
                    <section className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/55">
                        <div className="flex flex-col gap-4 border-b border-slate-800 p-5 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h2 className="text-lg font-bold text-white">Bảng xếp hạng câu lạc bộ</h2>
                                <p className="mt-1 text-xs text-slate-500">
                                    Đang hiển thị {filteredClubs.length} câu lạc bộ
                                </p>
                            </div>
                            <label className="relative block sm:w-72">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl border border-slate-700 bg-slate-950/60 py-2.5 pl-10 pr-4 text-sm text-white outline-none focus:border-cyan-400/50"
                                />
                            </label>
                        </div>
                            ...
                            <table className="min-w-[920px] w-full text-left text-sm">
                                <thead className="bg-slate-950/40 text-[11px] uppercase tracking-wider text-slate-500">
                                    <tr>
                                        ...
                                        <th className="px-5 py-4">Câu lạc bộ</th>
                                        <th className="px-5 py-4">Điểm</th>
                                        <th className="px-5 py-4">Xếp loại</th>
                                        <th className="px-5 py-4">Đã duyệt</th>
                                        <th className="px-5 py-4">Hoạt động</th>
                                        <th className="px-5 py-4">Người tham gia</th>
                                        <th className="px-5 py-4">Bị từ chối</th>
                                        <th className="px-5 py-4">Quá hạn</th>
                                    </tr>
                                </thead>
                ...
                <>
                    {clubs.length > 1 && (
                        <div className="flex items-center justify-between gap-4 rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
                            <label htmlFor="kpi-club" className="text-sm font-semibold text-slate-300">
                                Chọn câu lạc bộ
                            </label>
                            <select
                                id="kpi-club"
                                ...
                                className="min-w-56 rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-sm text-white outline-none focus:border-cyan-400/50"
                            >
                                {clubs.map((club) => (
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/50 p-5 sm:p-6">
                <div>
                    <h2 className="text-lg font-bold text-white">Quy tắc tính điểm KPI</h2>
                    <p className="mt-1 text-xs text-slate-500">Tổng điểm không bao giờ thấp hơn 0.</p>
                </div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 text-center text-xs sm:grid-cols-4">
                    {[
                        ...
                        <div key={rating} className="rounded-xl border border-slate-800 p-3">
                            <p className="font-bold text-white">{rating}</p>
                            <p className="mt-1 text-slate-500">{range} điểm</p>
                        </div>
                    ))}
                </div>
            </section>
