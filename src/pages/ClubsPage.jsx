...
const STATUS_STYLES = {
    ...
    APPROVED: 'border-emerald-500/30 bg-emerald-500/15 text-green-500',
...
const inputClass =
    'w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-neutral-900 outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100';
...
function getAdditionalFieldLabel(category, key) {
    return (
        (CATEGORY_FIELDS[category] || CATEGORY_FIELDS.OTHER).find(([fieldKey]) => fieldKey === key)?.[1] ||
        'Thông tin bổ sung'
    );
}
...
function StatusBadge({ status }) {
    ...
    return (
        <span
            className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${STATUS_STYLES[normalized] || STATUS_STYLES.SUBMITTED}`}
        >
...
export default function ClubsPage() {
    ...
    }, [canApplyForClub, canJoinClub, canReviewApplications, error, managedClubIds]);

    useEffect(() => {
        loadData();
    }, [loadData]);

    const membershipByClub = useMemo(() => new Map(memberships.map((item) => [item.clubId, item])), [memberships]);
    ...
    const pendingClubApplications = clubApplications.filter((item) => normalizedStatus(item.status) === 'SUBMITTED');
    ...
    const submitClubApplication = async (event) => {
        ...
        if (
            !clubForm.foundingMembersCommitted ||
    ...
    return (
        <div className="space-y-6">
            ...
            >
                <div>
                    ...
                    <p className="mt-1 max-w-3xl text-sm text-gray-400">
                        Quyền thành viên chỉ được kích hoạt sau khi chủ nhiệm câu lạc bộ phê duyệt. Bạn cũng có thể đăng
                        ký thành lập câu lạc bộ mới.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                {[
                    ...
                    [
                        ...
                        memberships.filter((item) => normalizedStatus(item.status) === 'APPROVED').length,
                        'text-green-500',
                    ],
                    [
                        'Đơn tham gia đang chờ duyệt',
                        memberships.filter((item) => normalizedStatus(item.status) === 'PENDING').length,
                        'text-amber-300',
                    ],
                    ['Đơn thành lập đang chờ duyệt', pendingClubApplications.length, 'text-violet-500'],
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-gray-500">
                                            Đã gửi: {formatDate(application.submittedAtUtc)}
                                        </p>
                                    </div>
                                ...
                                {application.reviewNote && (
                                    <p className="mt-3 rounded-lg bg-slate-900 p-3 text-sm text-gray-300">
                                        <span className="font-semibold text-gray-400">Nhận xét:</span>{' '}
                                        {application.reviewNote}
                                    </p>
                                )}
            ...
            {canReviewApplications && pendingClubApplications.length > 0 && (
                <section className="rounded-2xl border border-purple-500/25 bg-purple-500/5 p-5">
                    ...
                    <p className="mt-1 text-sm text-gray-400">
                        Kiểm tra toàn bộ thông tin đã gửi trước khi đưa ra quyết định.
                    </p>
                                ...
                                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                    <div>
                                        ...
                                        <p className="mt-2 text-sm text-gray-300">
                                            Người sáng lập: {application.requesterName} ·{' '}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <button
                                            ...
                                            onClick={(event) => {
                                                ...
                                                openApplicationReview(application, 'reject');
                                            }}
                                            className="rounded-lg border border-rose-500/30 px-3 py-2 text-sm font-semibold text-rose-300"
                                        >
                                            Từ chối
                                        </button>
                                        <button
                                            ...
                                            onClick={(event) => {
                                                event.stopPropagation();
                                                openApplicationReview(application, 'revision');
                                            }}
                                            className="rounded-lg border border-orange-500/30 px-3 py-2 text-sm font-semibold text-orange-300"
                                        >
                                            Yêu cầu chỉnh sửa
                                        </button>
                                        <button
                        ...
                        {pendingMemberReviews.map((membership) => (
                            ...
                            >
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-gray-400">
                                        {membership.clubName} · {membership.email} · {membership.phoneNumber}
                                    </p>
                                    <p className="mt-2 line-clamp-2 text-sm text-gray-300">{membership.reason}</p>
                                    <p className="mt-2 text-xs font-semibold text-green-500">Nhấn để xem toàn bộ đơn</p>
                                </div>
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        onClick={(event) => {
                                            event.stopPropagation();
                                            openMembershipReview(membership, 'reject');
                                        }}
                                        className="rounded-lg border border-rose-500/30 px-4 py-2 text-sm font-semibold text-rose-300"
                                    >
                                        Từ chối
                                    </button>
                                    <button
                                        ...
                                        onClick={(event) => {
                                            event.stopPropagation();
                                            openMembershipReview(membership, 'approve');
                                        }}
                            ...
                            >
                                <div className="flex-1 p-5">
                                    ...
                                    {status === 'REJECTED' && membership?.reviewNote && (
                                        <p className="mt-4 rounded-lg border border-rose-500/20 bg-rose-500/5 p-3 text-sm text-rose-200">
                                            Lý do: {membership.reviewNote}
                                        </p>
                                    )}
                                </div>
                                <div className="border-t border-outline bg-surface-soft p-4">
                                    {canDeleteClubs || isManaged ? (
                                        ...
                                        <p className="text-center text-sm font-semibold text-green-500">
                                            Tư cách thành viên đang hoạt động
                                        </p>
                                    ) : status === 'PENDING' ? (
                                        <p className="text-center text-sm font-semibold text-amber-300">
                                            Đơn đang chờ chủ nhiệm câu lạc bộ xét duyệt
                                        </p>
                                    ) : canJoinClub ? (
                                        <button
                                            type="button"
                                            onClick={() => openJoinForm(club)}
                                            className="w-full rounded-xl bg-gradient-to-r from-cyan-500 to-purple-500 px-4 py-3 font-semibold text-white"
                                        >
                                            {status === 'REJECTED' ? 'Gửi lại đơn' : 'Đăng ký tham gia'}
                                        </button>
                                    ) : (
                                        <p className="text-center text-sm text-gray-500">
                                            Tài khoản này không thể gửi đơn xin tham gia
                                        </p>
                                    )}
                                </div>
                            </motion.article>
            ...
            <Modal
                ...
                title={selectedClub ? `Đăng ký tham gia ${selectedClub.name}` : 'Đơn xin tham gia câu lạc bộ'}
                size="xl"
            >
                <form onSubmit={submitJoinRequest} className="space-y-6">
                    <p className="rounded-lg bg-cyan-50 p-3 text-sm text-cyan-900">
                        Việc gửi đơn chưa đồng nghĩa với việc trở thành thành viên. Quyền truy cập chỉ được kích hoạt
                        sau khi chủ nhiệm câu lạc bộ phê duyệt.
                    </p>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Họ và tên" required>
                                <TextInput
                                    ...
                                    onChange={(value) => updateJoinField('fullName', value)}
                                    required
                                    maxLength={200}
                                />
                            </Field>
                            <Field label="Ngày sinh" required>
                                <TextInput
                                    type="date"
                                    value={joinForm.dateOfBirth}
                                    onChange={(value) => updateJoinField('dateOfBirth', value)}
                                    required
                                />
                            </Field>
                            <Field label="Giới tính" required>
                                <select
                                    ...
                                    onChange={(event) => updateJoinField('gender', event.target.value)}
                                    required
                                    className={inputClass}
                                >
                            ...
                            <Field label="Thư điện tử" required>
                                <TextInput
                                    type="email"
                                    value={joinForm.email}
                                    onChange={(value) => updateJoinField('email', value)}
                                    required
                                    maxLength={255}
                                />
                            </Field>
                            <Field label="Số điện thoại" required>
                                <TextInput
                                    ...
                                    onChange={(value) => updateJoinField('phoneNumber', value)}
                                    required
                            ...
                            <Field label="Địa chỉ">
                                <TextInput
                                    ...
                                    onChange={(value) => updateJoinField('address', value)}
                                    maxLength={500}
                                />
                            </Field>
                        </div>
                    ...
                    <section className="space-y-4">
                        ...
                        <Checkbox
                            checked={joinForm.acceptedClubRules}
                            onChange={(value) => updateJoinField('acceptedClubRules', value)}
                            required
                        >
                            Tôi đã đọc và đồng ý tuân thủ nội quy câu lạc bộ
                        </Checkbox>
                        <Checkbox
                            checked={joinForm.committedToParticipate}
                            onChange={(value) => updateJoinField('committedToParticipate', value)}
                            required
                        >
                            Tôi cam kết tham gia đầy đủ các hoạt động của câu lạc bộ
                        </Checkbox>
                    </section>
                    ...
                    <section className="space-y-4">
                        <SectionTitle
                            ...
                            description={`Thông tin dành cho lĩnh vực ${CATEGORY_LABELS[selectedClub?.category] || 'Khác'}`}
                        />
                        <div className="grid gap-4 md:grid-cols-2">
                            {(CATEGORY_FIELDS[selectedClub?.category] || CATEGORY_FIELDS.OTHER).map(([key, label]) => (
                                <Field key={key} label={label}>
                                    <TextArea
                                        value={joinForm.additionalInfo[key] || ''}
                                        onChange={(value) =>
                                            setJoinForm((current) => ({
                                                ...current,
                                                additionalInfo: { ...current.additionalInfo, [key]: value },
                                            }))
                                        }
                                        rows={2}
                                    />
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Sở thích của bạn">
                                <TextArea
                                    value={joinForm.hobbies}
                                    onChange={(value) => updateJoinField('hobbies', value)}
                                />
                            </Field>
                            <Field label="Kỹ năng nổi bật">
                                <TextArea
                                    value={joinForm.skills}
                                    onChange={(value) => updateJoinField('skills', value)}
                                />
                            </Field>
                            <Field label="Lý do tham gia" required>
                                <TextArea
                                    value={joinForm.reason}
                                    onChange={(value) => updateJoinField('reason', value)}
                                    required
                                />
                            </Field>
                            <Field label="Bạn mong đợi điều gì từ câu lạc bộ?">
                                <TextArea
                                    value={joinForm.expectations}
                                    onChange={(value) => updateJoinField('expectations', value)}
                                />
                            </Field>
                            <Field label="Bạn có thể đóng góp gì cho câu lạc bộ?">
                                <TextArea
                                    value={joinForm.contributions}
                                    onChange={(value) => updateJoinField('contributions', value)}
                                />
                            </Field>
                            <Field label="Lời nhắn bổ sung">
                                <TextArea
                                    value={joinForm.message}
                                    onChange={(value) => updateJoinField('message', value)}
                                />
                            </Field>
                        </div>
                    </section>
                    <div className="sticky -bottom-6 z-10 -mx-6 flex gap-3 border-t border-outline bg-surface px-6 py-4 shadow-lg">
                        <button
                            ...
                            className="flex-1 rounded-lg bg-neutral-100 px-4 py-3 font-semibold text-neutral-700"
                        >
                            Hủy
                        </button>
                        <button
            ...
            <Modal
                ...
                onClose={() => !isSubmitting && setCreationModalOpen(false)}
                title={editingApplication ? 'Chỉnh sửa đơn thành lập câu lạc bộ' : 'Đơn thành lập câu lạc bộ'}
                size="xl"
            >
                <form onSubmit={submitClubApplication} className="space-y-7">
                    <section className="space-y-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Tên câu lạc bộ đề xuất" required>
                                <TextInput
                                    value={clubForm.name}
                                    onChange={(value) => updateClubForm('name', value)}
                                    required
                                    maxLength={200}
                                />
                            </Field>
                            <Field label="Lĩnh vực hoạt động" required>
                                <select
                                    ...
                                    onChange={(event) => updateClubForm('category', event.target.value)}
                                    required
                                    className={inputClass}
                                >
                            ...
                            <Field label="Tên viết tắt (không bắt buộc)">
                                <TextInput
                                    value={clubForm.code}
                                    onChange={(value) => updateClubForm('code', value.toUpperCase())}
                                    maxLength={20}
                        ...
                        <Field label="Mục tiêu" required>
                            <TextArea
                                value={clubForm.purpose}
                                onChange={(value) => updateClubForm('purpose', value)}
                                required
                            />
                        </Field>
                        <Field label="Mô tả ngắn về câu lạc bộ" required>
                            <TextArea
                                value={clubForm.description}
                                onChange={(value) => updateClubForm('description', value)}
                                required
                            />
                        </Field>
                    </section>
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Họ và tên" required>
                                <TextInput
                                    value={clubForm.founderFullName}
                                    onChange={(value) => updateClubForm('founderFullName', value)}
                                    required
                                />
                            </Field>
                            <Field label="Vai trò đề xuất" required>
                                <TextInput
                                    value={clubForm.founderRole}
                                    onChange={(value) => updateClubForm('founderRole', value)}
                                    required
                                />
                            </Field>
                            <Field label="Thư điện tử" required>
                                <TextInput
                                    type="email"
                                    value={clubForm.founderEmail}
                                    onChange={(value) => updateClubForm('founderEmail', value)}
                                    required
                                />
                            </Field>
                            <Field label="Số điện thoại" required>
                                <TextInput
                                    type="tel"
                                    value={clubForm.founderPhone}
                                    onChange={(value) => updateClubForm('founderPhone', value)}
                                    required
                                />
                            </Field>
                            <Field label="Lớp / Khoa / Phòng ban" required>
                                <TextInput
                                    value={clubForm.founderOrganization}
                                    onChange={(value) => updateClubForm('founderOrganization', value)}
                                    required
                                />
                            </Field>
                        </div>
                    ...
                    <section className="space-y-4">
                        ...
                        <Field label="Số thành viên sáng lập" required>
                            <input
                                type="number"
                                min="1"
                                max="200"
                                value={clubForm.foundingMemberCount}
                                onChange={(event) => updateClubForm('foundingMemberCount', Number(event.target.value))}
                                required
                                className={inputClass}
                            />
                        </Field>
                            ...
                            {clubForm.foundingMembers.map((member, index) => (
                                ...
                                >
                                    <div className="mb-3 flex items-center justify-between">
                                        ...
                                        {clubForm.foundingMembers.length > 1 && (
                                            <button
                                                type="button"
                                    ...
                                    <div className="grid gap-3 md:grid-cols-3">
                                        <TextInput
                                            ...
                                            onChange={(value) => updateFoundingMember(index, 'fullName', value)}
                                            required
                                            placeholder="Họ và tên"
                                            ariaLabel={`Họ và tên thành viên sáng lập ${index + 1}`}
                                        />
                                        <TextInput
                                            value={member.organization}
                                            onChange={(value) => updateFoundingMember(index, 'organization', value)}
                                            required
                                            placeholder="Lớp/Khoa"
                                            ariaLabel={`Lớp hoặc khoa của thành viên sáng lập ${index + 1}`}
                                        />
                                        <TextInput
                                            type="email"
                                            value={member.email}
                                            onChange={(value) => updateFoundingMember(index, 'email', value)}
                                            required
                                            placeholder="Thư điện tử"
                                            ariaLabel={`Thư điện tử của thành viên sáng lập ${index + 1}`}
                                        />
                                    </div>
                                </div>
                            ))}
                        ...
                        <Checkbox
                            checked={clubForm.foundingMembersCommitted}
                            onChange={(value) => updateClubForm('foundingMembersCommitted', value)}
                            required
                        >
                            Các thành viên sáng lập đã đồng ý và cam kết tham gia
                        </Checkbox>
                    </section>

                    <section className="space-y-4">
                        <SectionTitle number="4" title="Kế hoạch hoạt động" />
                        <Field label="Các hoạt động chính dự kiến" required>
                            <TextArea
                                value={clubForm.mainActivities}
                                onChange={(value) => updateClubForm('mainActivities', value)}
                                required
                                rows={4}
                                maxLength={2000}
                            />
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Tần suất hoạt động" required>
                                <div className="grid grid-cols-[minmax(0,1fr)_130px] gap-2">
                                    ...
                                    <select
                                        ...
                                        onChange={(event) =>
                                            updateClubForm('activityFrequencyUnit', event.target.value)
                                        }
                            ...
                            <Field label="Địa điểm đề xuất">
                                <TextInput
                                    value={clubForm.expectedLocation}
                                    onChange={(value) => updateClubForm('expectedLocation', value)}
                                />
                            </Field>
                            <Field label="Lịch dự kiến">
                                <TextInput
                                    value={clubForm.expectedSchedule}
                                    onChange={(value) => updateClubForm('expectedSchedule', value)}
                                    placeholder="Khung giờ/ngày"
                                />
                            </Field>
                            <Field label="Sự kiện lớn dự kiến">
                                <TextArea
                                    value={clubForm.majorEvents}
                                    onChange={(value) => updateClubForm('majorEvents', value)}
                                />
                            </Field>
                        </div>
                    </section>
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <Field label="Phòng họp / địa điểm">
                                <select
                                    value={clubForm.venueSupport}
                                    onChange={(event) => updateClubForm('venueSupport', event.target.value)}
                                    className={inputClass}
                                >
                                    <option value="SUPPORT_NEEDED">Cần hỗ trợ</option>
                                    <option value="SELF_MANAGED">Tự quản lý</option>
                                </select>
                            </Field>
                            <Field label="Kinh phí">
                                <select
                                    value={clubForm.fundingSupport}
                                    onChange={(event) => updateClubForm('fundingSupport', event.target.value)}
                                    className={inputClass}
                                >
                                    <option value="SUPPORT_NEEDED">Cần hỗ trợ</option>
                                    <option value="SELF_FUNDED">Tự tài trợ</option>
                                    <option value="COMBINED">Kết hợp</option>
                                </select>
                            </Field>
                            <Field label="Thiết bị cần thiết">
                                <TextArea
                                    value={clubForm.equipmentNeeds}
                                    onChange={(value) => updateClubForm('equipmentNeeds', value)}
                                />
                            </Field>
                            <Checkbox
                                checked={clubForm.advisorNeeded}
                                onChange={(value) => updateClubForm('advisorNeeded', value)}
                            >
                                Cần cố vấn hoặc người hướng dẫn
                            </Checkbox>
                        </div>
                    </section>
                    ...
                    <section className="space-y-3">
                        ...
                        <Checkbox
                            checked={clubForm.committedToRules}
                            onChange={(value) => updateClubForm('committedToRules', value)}
                            required
                        >
                            Đảm bảo mọi hoạt động tuân thủ quy định hiện hành
                        </Checkbox>
                        <Checkbox
                            checked={clubForm.committedToResponsibility}
                            onChange={(value) => updateClubForm('committedToResponsibility', value)}
                            required
                        >
                            Chịu trách nhiệm trước ban quản trị
                        </Checkbox>
                        <Checkbox
                            checked={clubForm.committedToReporting}
                            onChange={(value) => updateClubForm('committedToReporting', value)}
                            required
                        >
                            Nộp báo cáo hoạt động định kỳ
                        </Checkbox>
                    </section>
                    <div className="sticky -bottom-6 z-10 -mx-6 flex gap-3 border-t border-outline bg-surface px-6 py-4 shadow-lg">
                        <button
                        ...
                        <button
                            type="submit"
                            disabled={isSubmitting}
                            className="flex-1 rounded-lg bg-gradient-to-r from-purple-600 to-fuchsia-600 px-4 py-3 font-semibold text-white disabled:opacity-50"
                        >
                            {isSubmitting ? 'Đang gửi...' : editingApplication ? 'Gửi lại đơn' : 'Gửi đơn thành lập'}
                        </button>
                    </div>
                </form>
            </Modal>
            ...
            <Modal
                isOpen={Boolean(membershipReview)}
                onClose={() => !isSubmitting && setMembershipReview(null)}
                title={membershipReviewAction === 'approve' ? 'Xét duyệt đơn tham gia' : 'Từ chối đơn tham gia'}
                size="lg"
            >
                <form onSubmit={submitMembershipReview} className="space-y-5">
                    {membershipReview && (
                        <div className="grid gap-4 rounded-xl bg-neutral-50 p-4 md:grid-cols-2">
                            ...
                            <Detail
                                label="Giới tính"
                                value={GENDER_LABELS[membershipReview.gender] || 'Không xác định'}
                            />
                            ...
                            <Detail
                                label="Cam kết tuân thủ nội quy"
                                value={membershipReview.acceptedClubRules ? 'Đã xác nhận' : 'Chưa xác nhận'}
                            />
                            <Detail
                                label="Cam kết tham gia hoạt động"
                                value={membershipReview.committedToParticipate ? 'Đã xác nhận' : 'Chưa xác nhận'}
                            />
                    ...
                    <Field
                        label={membershipReviewAction === 'reject' ? 'Lý do từ chối' : 'Nhận xét xét duyệt'}
                        required={membershipReviewAction === 'reject'}
                    >
                        <TextArea
                            value={membershipReviewNote}
                            onChange={setMembershipReviewNote}
                            required={membershipReviewAction === 'reject'}
                        />
                    </Field>
                    <div className="flex gap-3">
                        <button
                            type="button"
                            onClick={() => setMembershipReview(null)}
                            className="flex-1 rounded-lg bg-neutral-100 px-4 py-3 font-semibold text-neutral-700"
                        >
                            Hủy
                        </button>
                        <button
                            type="submit"
                            disabled={isSubmitting}
                            className={`flex-1 rounded-lg px-4 py-3 font-semibold text-white ${membershipReviewAction === 'approve' ? 'bg-emerald-600' : 'bg-rose-600'}`}
                        >
            ...
            <Modal
                isOpen={Boolean(applicationReview)}
                onClose={() => !isSubmitting && setApplicationReview(null)}
                title="Xét duyệt của quản trị viên"
                size="xl"
            >
                <form onSubmit={submitApplicationReview} className="space-y-6">
                    {applicationReview && (
                        <div className="space-y-5">
                            <div className="grid gap-4 rounded-xl bg-neutral-50 p-4 md:grid-cols-3">
                                ...
                                <Detail
                                    label="Lĩnh vực"
                                    value={CATEGORY_LABELS[applicationReview.category] || 'Khác'}
                                />
                                ...
                                <Detail
                                    ...
                                    value={`${applicationReview.requesterName} – ${applicationReview.founderRole}`}
                                />
                                <Detail
                                    label="Liên hệ"
                                    value={`${applicationReview.contactEmail} · ${applicationReview.contactPhone}`}
                                />
                                ...
                                <Detail
                                    label="Địa điểm/lịch"
                                    value={`${applicationReview.expectedLocation || '—'} · ${applicationReview.expectedSchedule || '—'}`}
                                />
                                <Detail label="Sự kiện lớn" value={applicationReview.majorEvents} />
                                <Detail
                                    label="Phòng họp / địa điểm"
                                    value={VENUE_SUPPORT_LABELS[applicationReview.venueSupport] || 'Không xác định'}
                                />
                                <Detail
                                    label="Kinh phí"
                                    value={FUNDING_SUPPORT_LABELS[applicationReview.fundingSupport] || 'Không xác định'}
                                />
                                ...
                                <section className="rounded-xl border border-neutral-200 p-4">
                                    ...
                                    <p className="mt-3 text-sm text-neutral-700">
                                        ...
                                        <span className="font-semibold">
                                            {applicationReview.foundingMembersCommitted
                                                ? 'Đã xác nhận'
                                                : 'Chưa xác nhận'}
                                        </span>
                                    </p>
                                ...
                                <section className="rounded-xl border border-neutral-200 p-4">
                                    ...
                                    <div className="mt-3 space-y-2 text-sm text-neutral-700">
                                        <p>
                                            Tuân thủ quy định:{' '}
                                            <span className="font-semibold">
                                                {applicationReview.committedToRules ? 'Đã xác nhận' : 'Chưa xác nhận'}
                                            </span>
                                        </p>
                                        <p>
                                            Chịu trách nhiệm quản lý:{' '}
                                            <span className="font-semibold">
                                                {applicationReview.committedToResponsibility
                                                    ? 'Đã xác nhận'
                                                    : 'Chưa xác nhận'}
                                            </span>
                                        </p>
                                        <p>
                                            Nộp báo cáo hoạt động định kỳ:{' '}
                                            <span className="font-semibold">
                                                {applicationReview.committedToReporting
                                                    ? 'Đã xác nhận'
                                                    : 'Chưa xác nhận'}
                                            </span>
                                        </p>
                            ...
                            <div>
                                ...
                                <div className="grid gap-2 sm:grid-cols-3">
                                    {[
                                        ...
                                        <label
                                            key={value}
                                            className={`cursor-pointer rounded-lg border p-3 text-center font-semibold ${applicationReviewAction === value ? 'border-cyan-500 bg-cyan-50 text-cyan-700' : 'border-neutral-200 text-neutral-600'}`}
                                        >
                                            <input
                                                type="radio"
                                                name="decision"
                                                value={value}
                                                checked={applicationReviewAction === value}
                                                onChange={() => setApplicationReviewAction(value)}
                                                className="sr-only"
                                            />
                                            {label}
                                        </label>
                    ...
                    <Field
                        label={applicationReviewAction === 'approve' ? 'Ghi chú' : 'Nhận xét / nội dung cần chỉnh sửa'}
                        required={applicationReviewAction !== 'approve'}
                    >
                        <TextArea
                            value={applicationReviewForm.note}
                            onChange={(value) => setApplicationReviewForm((current) => ({ ...current, note: value }))}
                            required={applicationReviewAction !== 'approve'}
                        />
                    </Field>
                    <Field label="Điều kiện">
                        <TextArea
                            value={applicationReviewForm.conditions}
                            onChange={(value) =>
                                setApplicationReviewForm((current) => ({ ...current, conditions: value }))
                            }
                        />
                    </Field>
                    <Field label="Chữ ký người xét duyệt">
                        <TextInput
                            value={applicationReviewForm.reviewerSignature}
                            onChange={(value) =>
                                setApplicationReviewForm((current) => ({ ...current, reviewerSignature: value }))
                            }
                            maxLength={200}
                        />
                    </Field>
                    <div className="flex gap-3">
                        <button
                            type="button"
                            onClick={() => setApplicationReview(null)}
                            className="flex-1 rounded-lg bg-neutral-100 px-4 py-3 font-semibold text-neutral-700"
                        >
                            Hủy
                        </button>
            ...
            >
                <div className="space-y-5">
                    <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900">
                        ...
                        <p className="mt-2">
                            Câu lạc bộ sẽ bị vô hiệu hóa và xóa khỏi danh sách đang hoạt động. Các phân công chủ nhiệm
                            hiện tại cũng sẽ kết thúc.
                        </p>
                    </div>
                    <div className="flex gap-3">
                        <button
