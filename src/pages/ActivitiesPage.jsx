        ...
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">
            {WEEK_DAYS.map(([day, label]) => (
                <button
                    ...
                    type="button"
                    onClick={() => toggle(day)}
                    className={`rounded-lg border px-2 py-2.5 text-sm font-semibold transition ${value.includes(day) ? 'border-cyan-500 bg-cyan-500 text-white' : 'border-neutral-300 bg-white text-neutral-600 hover:border-cyan-400'}`}
                >
                    {label}
                </button>
            ))}
...
function formatMeetingDays(days = []) {
    return days
        .map((day) => WEEK_DAYS.find((item) => item[0] === day)?.[1])
        .filter(Boolean)
        .join(', ');
}
...
export default function ActivitiesPage() {
    ...
    const accessByClub = useMemo(() => new Map(clubAccess.map((access) => [access.clubId, access])), [clubAccess]);
    ...
    const createActivity = async (event) => {
        ...
        try {
            ...
            setActivities((current) =>
                [...current, created].sort((a, b) => new Date(a.startTimeUtc) - new Date(b.startTimeUtc)),
            );
            setShowCreate(false);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                {[
                    ...
                    [
                        'Đã hoàn thành',
                        activities.filter((item) => normalizeStatus(item.status) === 'COMPLETED').length,
                        'text-green-500',
                    ],
                    [
                        'Lịch họp hằng tuần',
                        activities.filter((item) => item.meetingDays?.length).length,
                        'text-amber-300',
                    ],
            ...
            {isLoading ? (
                ...
                <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                    {filteredActivities.map((activity, index) => {
                        ...
                        const registered = activity.participants?.some(
                            ...
                            >
                                    ...
                                    <span
                                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                                            status === 'COMPLETED'
                                                ...
                                                : 'bg-purple-500/15 text-violet-500'
                                        }`}
                                    >
                                ...
                                <dl className="mt-4 space-y-2 border-t border-slate-800 pt-4 text-sm">
                                    <div className="flex justify-between gap-3">
                                        <dt className="text-gray-500">{isWeekly ? 'Ngày họp' : 'Bắt đầu'}</dt>
                                        <dd className="text-right text-gray-200">
                                            {isWeekly
                                                ? formatMeetingDays(activity.meetingDays)
                                                : formatDate(activity.startTimeUtc)}
                                        </dd>
                                    </div>
                                    <div className="flex justify-between gap-3">
                                        <dt className="text-gray-500">Địa điểm</dt>
                                        <dd className="text-right text-gray-200">{activity.location}</dd>
                                    </div>
                                    <div className="flex justify-between gap-3">
                                        <dt className="text-gray-500">Đã đăng ký</dt>
                                ...
                                <div className="mt-auto flex gap-2 pt-5">
                                    ...
                                    {isWeekly && status !== 'COMPLETED' && canParticipate ? (
                                        <button
                                            ...
                                            className="flex-1 rounded-xl bg-emerald-500/15 px-3 py-2.5 text-sm font-semibold text-green-500 disabled:opacity-45"
                                        >
                                            {busyId === `check-in-${activity.id}`
                                                ? 'Đang điểm danh...'
                                                : 'Điểm danh hôm nay'}
                                        </button>
                                    ) : (
            ...
            <Modal
                ...
                title="Tạo hoạt động hoặc lịch họp hằng tuần"
                size="lg"
            >
                <form onSubmit={createActivity} className="space-y-4">
                    <FormField label="Loại *">
                        <select
                            value={formData.kind}
                            onChange={(event) => setFormData((current) => ({ ...current, kind: event.target.value }))}
                    ...
                    {formData.kind === 'WEEKLY' ? (
                        <FormField label="Ngày họp hằng tuần theo giờ Việt Nam *">
                            <MeetingDayPicker
                                ...
                                onChange={(meetingDays) => setFormData((current) => ({ ...current, meetingDays }))}
                            />
                            <span className="mt-2 block text-xs font-normal text-neutral-500">
                                Thành viên được điểm danh một lần vào mỗi ngày đã chọn.
                            </span>
                        </FormField>
                    ) : (
                    ...
                    <div className="space-y-4 text-neutral-700">
                        ...
                        <dl className="grid gap-3 sm:grid-cols-2">
                            <div className="rounded-lg bg-neutral-50 p-3">
                                <dt className="text-xs text-neutral-500">Câu lạc bộ</dt>
                                <dd className="mt-1 font-semibold">{selectedActivity.clubName}</dd>
                            </div>
                            <div className="rounded-lg bg-neutral-50 p-3">
                                <dt className="text-xs text-neutral-500">Địa điểm</dt>
                                <dd className="mt-1 font-semibold">{selectedActivity.location}</dd>
                            </div>
                            {selectedActivity.meetingDays?.length ? (
                                <div className="rounded-lg bg-neutral-50 p-3 sm:col-span-2">
                                    <dt className="text-xs text-neutral-500">Ngày họp hằng tuần, giờ Việt Nam</dt>
                                    <dd className="mt-1 font-semibold">
                                        {formatMeetingDays(selectedActivity.meetingDays)}
                                    </dd>
                                </div>
                            ) : (
                                <>
                                    <div className="rounded-lg bg-neutral-50 p-3">
                                        <dt className="text-xs text-neutral-500">Bắt đầu</dt>
                                        <dd className="mt-1 font-semibold">
                                            {formatDate(selectedActivity.startTimeUtc)}
                                        </dd>
                                    </div>
                                    <div className="rounded-lg bg-neutral-50 p-3">
                                        <dt className="text-xs text-neutral-500">Kết thúc</dt>
                                        <dd className="mt-1 font-semibold">
                                            {formatDate(selectedActivity.endTimeUtc)}
                                        </dd>
                                    </div>
                                </>
                            )}
                        ...
                        {selectedActivity.meetingDays?.length && (
                            <section className="space-y-3 rounded-xl border border-neutral-200 p-4">
                                <div className="flex items-center justify-between">
                                    <h3 className="font-bold">Điểm danh của tôi</h3>
                                    {summaryLoading && <span className="text-xs text-neutral-500">Đang tải...</span>}
                                </div>
                                {attendanceSummary && (
                                    <>
                            ...
                            selectedActivity.meetingDays?.length > 0 && (
                                <button
                                    ...
                                    onClick={() => {
                                        setEditingSchedule({
                                            ...selectedActivity,
                                            meetingDays: [...selectedActivity.meetingDays],
                                        });
                                        setSelectedActivity(null);
                                    }}
                                    className="w-full rounded-lg bg-cyan-600 px-4 py-3 font-semibold text-white"
                                >
                                    Chỉnh sửa lịch họp hằng tuần
                                </button>
                            )}
                        {accessByClub.get(selectedActivity.clubId)?.isManager &&
            ...
            <Modal
                ...
                title="Chỉnh sửa lịch họp hằng tuần"
                size="lg"
            >
                {editingSchedule && (
                    <form onSubmit={saveSchedule} className="space-y-5">
                        <div>
                            <p className="font-semibold text-neutral-900">{editingSchedule.title}</p>
                            <p className="text-sm text-neutral-500">Chọn các ngày thành viên được phép điểm danh.</p>
                        </div>
                        <MeetingDayPicker
                            value={editingSchedule.meetingDays}
                            onChange={(meetingDays) => setEditingSchedule((current) => ({ ...current, meetingDays }))}
                        />
                        <div className="flex gap-3">
                            <button
                                type="button"
                            ...
                            <button
                                type="submit"
                                disabled={busyId === `schedule-${editingSchedule.id}`}
                                className="flex-1 rounded-lg bg-cyan-600 px-4 py-3 font-semibold text-white disabled:opacity-50"
                            >
                                {busyId === `schedule-${editingSchedule.id}` ? 'Đang lưu...' : 'Lưu lịch họp'}
                            </button>
                        </div>
                    </form>
                )}
