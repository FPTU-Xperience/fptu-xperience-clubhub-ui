...
function ReviewDecisionModal({
    ...
    const title = isApproval ? (isManagerReview ? 'Xét duyệt báo cáo' : 'Phê duyệt báo cáo') : 'Yêu cầu chỉnh sửa';
            ...
            <div className="space-y-4">
                ...
                >
                    ...
                    <p>
                        {isApproval
                            ? isManagerReview
                                ? 'Báo cáo sẽ được chuyển đến quản trị viên để phê duyệt cuối.'
                ...
                <div>
                    <label className="mb-2 block text-sm font-semibold text-slate-800">
                        Nhận xét {!isApproval && <span className="text-rose-600">*</span>}
                    </label>
                    <textarea
                        rows={5}
                        value={feedbackNote}
                        ...
                        placeholder={
                            isApproval ? 'Nhận xét tích cực hoặc lưu ý thêm (không bắt buộc)' : 'Nội dung cần chỉnh sửa'
                        }
                        maxLength={1000}
...
export default function ReportDetailPage() {
    ...
    const canSubmit = isReportAuthor && (isClubManager || isUploadedReport) && ['DRAFT', 'REJECTED'].includes(status);
    const isReviewer = hasRole('ADMIN') || hasRole('STUDENT_AFFAIRS_ADMIN');

    const isFutureEvent = report?.reportType === 'FUTURE_EVENT' || report?.tag === 'FUTURE_EVENT';
    const canManagerReview = isClubManager && status === 'SUBMITTED' && (isFutureEvent || !isReportAuthor);
    const canFinalReview = isReviewer && status === 'UNDERREVIEW';
    const canReview = canManagerReview || canFinalReview;

    // Stable callback for fetching preview — avoids effect reloading on parent re-renders
    const fetchPreview = useCallback((reportId) => api.getUploadedReportFilePreview(reportId), []);
        ...
        try {
            if (reviewAction === 'approve') {
                if (canManagerReview) {
                    ...
                    success('Đã duyệt và chuyển báo cáo đến quản trị viên.');
                } else {
        ...
        return (
            <>
                <MinimalUploadedReportPreview
                    ...
                    canReview={canReview}
                    reviewStage={canManagerReview ? 'manager' : 'final'}
                ...
                <ReviewDecisionModal
                    ...
                    isManagerReview={canManagerReview}
                />
            </>
        );
    }
            ...
            >
                <div className="rounded-2xl border border-slate-800 bg-slate-900/90 p-6 sm:p-12 shadow-2xl space-y-8 text-slate-200">
                    ...
                    <div className="border-b border-slate-800 pb-6 text-center space-y-3">
                        ...
                        <h1 className="font-sans text-2xl font-bold tracking-wide sm:text-3xl uppercase pt-2">
                            BÁO CÁO HOẠT ĐỘNG {report.period}
                        </h1>
                        <h2 className="text-lg font-semibold text-cyan-300">{report.clubName}</h2>
                        <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-slate-400 pt-2 font-sans">
                            <span>
                                Mã báo cáo: <strong className="text-slate-200">#{report.id}</strong>
                            </span>
                            <span>•</span>
                            <span>
                                Loại: <strong className="text-slate-200">{formatReportType(report.reportType)}</strong>
                            </span>
                            <span>•</span>
                            <span>
                                Phân loại: <strong className="text-slate-200">{formatReportType(report.tag)}</strong>
                            </span>
                        </div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-2 rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-left text-xs sm:grid-cols-4">
                            ...
                            <div>
                                ...
                                <span className="mt-1 block font-medium text-slate-300">
                                    {formatDate(report.submittedAtUtc, true)}
                                </span>
                            </div>
                            <div>
                                <span className="block text-slate-500">Ngày xét duyệt</span>
                                <span className="mt-1 block font-medium text-slate-300">
                                    {formatDate(report.reviewedAtUtc, true)}
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        {!report.details || report.details.length === 0 ? (
                            ...
                            <div className="space-y-6 pt-1">
                                {report.details.map((act, index) => (
                                    <div
                                        key={act.id || index}
                                        className="rounded-xl border border-slate-800 bg-slate-950/40 p-5 space-y-3 text-sm"
                                    >
                                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-2">
                                            ...
                                            {act.budgetSpent > 0 && (
                                                <span className="font-sans text-xs font-semibold text-amber-300">
                                        ...
                                        <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4 text-slate-400">
                                            <div>
                                                ...
                                                <span className="font-medium text-slate-200">
                                                    {act.objective || 'Không áp dụng'}
                                                </span>
                                            </div>
                                            <div>
                                                <span className="block text-slate-500">Mục tiêu tham gia</span>
                                                <span className="font-medium text-slate-200">
                                                    {act.targetParticipantCount || 0} người
                                                </span>
                                            </div>
                                            <div>
                                        ...
                                        {act.evidenceUrl && (
                                            <div className="pt-2">
                                                ...
                                                >
                                                    <Paperclip size={14} /> Liên kết minh chứng: {act.evidenceUrl}{' '}
                    ...
                    <section className="space-y-3">
                        ...
                        {!report.feedback || report.feedback.length === 0 ? (
                            <p className="text-sm text-slate-500 py-2">
                                Báo cáo chưa có ý kiến phản hồi từ người duyệt.
                            </p>
                        ) : (
                            <div className="space-y-3 pt-1">
                                {report.feedback.map((fb) => (
                                    <div
                                        key={fb.id}
                                        className="rounded-xl border border-slate-800 bg-slate-950/70 p-4 text-xs space-y-2"
                                    >
