...
function statusClass(status) {
    ...
    if (value === 'APPROVED' || value === 'SETTLED') return 'bg-emerald-500/15 text-green-500';
    if (value === 'REJECTED') return 'bg-rose-500/15 text-rose-300';
    if (value === 'MANAGERAPPROVED') return 'bg-purple-500/15 text-violet-500';
...
function isBudgetActivity(activity) {
    ...
    return !['attendance', 'roll call', 'check-in', 'check in', 'điểm danh', 'diem danh'].some((keyword) =>
...
export default function FinancePage() {
    ...
    const eligibleActivities = useMemo(
        () =>
            activities.filter(
                (activity) => activity.clubId === Number(proposalForm.clubId) && isBudgetActivity(activity),
            ),
        [activities, proposalForm.clubId],
    );
    const eligibleFutureReports = useMemo(
        () =>
            futureReports.filter(
                (report) =>
    ...
    const createProposal = async (event) => {
        ...
        if (
            !club ||
            !proposalTitle ||
            (!isOther && !selectedActivity && !selectedReport) ||
            !proposalForm.description.trim() ||
            requestedAmount <= 0
        ) {
            error(
                'Vui lòng chọn hoạt động, báo cáo sự kiện sắp tới hoặc mục Khác, sau đó nhập đầy đủ thông tin đề xuất.',
            );
            return;
        }
        setBusyId('create');
    ...
    const submitReview = async (event) => {
        ...
        try {
            ...
            success(
                reviewAction === 'approve'
                    ? reviewStage === 'manager'
                        ? 'Đề xuất đã được chuyển đến bước duyệt cuối.'
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-400">
                        Chỉ thủ quỹ câu lạc bộ đã được duyệt mới có thể gửi đề xuất và quyết toán.
                    </p>
                </div>
                ...
                {[
                    ...
                    [
                        ...
                        proposals.filter((item) =>
                            ['SUBMITTED', 'MANAGERAPPROVED'].includes(normalizeStatus(item.status)),
                        ).length,
                        'text-amber-300',
                    ],
                    ['Ngân sách đã duyệt', money(approvedBudget), 'text-green-500'],
                    ['Số tiền đã quyết toán', money(spent), 'text-violet-500'],
            ...
            <select
                aria-label="Lọc đề xuất ngân sách theo trạng thái"
                value={statusFilter}
                onChange={(event) => setStatusFilter(event.target.value)}
                className="w-full rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-white sm:w-auto"
            >
            ...
            {isLoading ? (
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                    {filtered.map((proposal) => {
                        ...
                        const activeSettlement = proposal.settlements?.find((item) =>
                            ['SUBMITTED', 'APPROVED'].includes(normalizeStatus(item.status)),
                        );
                        return (
                            <article
                                key={proposal.id}
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <h3 className="mt-1 text-lg font-bold text-white">{proposal.title}</h3>
                                    </div>
                                    <span
                                        className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${statusClass(status)}`}
                                    >
                                ...
                                {proposal.activityId && (
                                    <p className="mt-3 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-2 text-xs text-cyan-300">
                                        Hoạt động liên kết: {linkedActivity?.title || proposal.title} · #
                                        {proposal.activityId}
                                    </p>
                                )}
                                {proposal.sourceReportId && (
                                    <div className="mt-3 flex items-center justify-between gap-3 rounded-lg border border-purple-500/20 bg-purple-500/5 px-3 py-2 text-xs text-purple-200">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => navigate(`/reports/${proposal.sourceReportId}`)}
                                            className="font-semibold text-violet-500 hover:text-purple-200"
                                        >
                                            Mở báo cáo
                                        </button>
                                    </div>
                                )}
                                {!proposal.activityId && !proposal.sourceReportId && (
                                    <p className="mt-3 text-xs text-gray-500">Tên đề xuất tự nhập</p>
                                )}
                                <p className="mt-3 text-sm leading-6 text-gray-400">{proposal.description}</p>
                                <dl className="mt-4 grid grid-cols-2 gap-3">
                                    <div className="rounded-xl bg-slate-950/60 p-3">
                                        <dt className="text-xs text-gray-500">Đề xuất</dt>
                                        <dd className="mt-1 font-bold text-white">{money(proposal.requestedAmount)}</dd>
                                    </div>
                                    <div className="rounded-xl bg-slate-950/60 p-3">
                                        <dt className="text-xs text-gray-500">Đã duyệt</dt>
                                        <dd className="mt-1 font-bold text-green-500">
                                            {proposal.approvedAmount ? money(proposal.approvedAmount) : '—'}
                                        </dd>
                                    </div>
                                </dl>
                                {proposal.managerReviewNote && (
                                    <p className="mt-3 rounded-lg bg-purple-500/5 p-3 text-sm text-purple-200">
                                        Chủ nhiệm: {formatSystemText(proposal.managerReviewNote)}
                                    </p>
                                )}
                                {proposal.reviewNote && (
                                    <p className="mt-3 rounded-lg bg-slate-950/50 p-3 text-sm text-gray-300">
                                        Xét duyệt cuối: {formatSystemText(proposal.reviewNote)}
                                    </p>
                                )}
                                {proposal.settlements?.length > 0 && (
                                    <div className="mt-4 space-y-2 border-t border-slate-800 pt-4">
                                        {proposal.settlements.map((settlement) => (
                                            <div
                                                key={settlement.id}
                                                className="flex flex-col gap-2 rounded-lg bg-slate-950/50 p-3 sm:flex-row sm:items-center sm:justify-between"
                                            >
                                                <div>
                                                    <p className="text-sm font-semibold text-white">
                                                        Quyết toán {money(settlement.totalSpent)}
                                                    </p>
                                ...
                                <div className="mt-5 flex flex-wrap gap-2">
                                    {proposal.sourceReportId && ['SUBMITTED', 'MANAGERAPPROVED'].includes(status) && (
                                        <button
                                            type="button"
                                            onClick={() => navigate(`/reports/${proposal.sourceReportId}`)}
                                            className="rounded-lg bg-purple-500/15 px-3 py-2 text-sm font-semibold text-violet-500"
                                        >
                                            Xét duyệt báo cáo kết hợp
                                        </button>
                                    )}
                                    {!proposal.sourceReportId && isManagerClub && status === 'SUBMITTED' && (
                                        <>
                                            <button
                                                type="button"
                                                onClick={() => openReview(proposal, 'reject', 'manager')}
                                                className="rounded-lg bg-rose-500/15 px-3 py-2 text-sm font-semibold text-rose-300"
                                            >
                                                Chủ nhiệm từ chối
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => openReview(proposal, 'approve', 'manager')}
                                                className="rounded-lg bg-purple-500/15 px-3 py-2 text-sm font-semibold text-violet-500"
                                            >
                                                Chủ nhiệm phê duyệt
                                            </button>
                                    ...
                                    {!proposal.sourceReportId && canFinalReview && status === 'MANAGERAPPROVED' && (
                                        <>
                                            <button
                                                type="button"
                                                onClick={() => openReview(proposal, 'reject', 'final')}
                                                className="rounded-lg bg-rose-500/15 px-3 py-2 text-sm font-semibold text-rose-300"
                                            >
                                                Từ chối cuối
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => openReview(proposal, 'approve', 'final')}
                                                className="rounded-lg bg-emerald-500/15 px-3 py-2 text-sm font-semibold text-green-500"
                                            >
                                                Phê duyệt cuối
                                            </button>
                                        </>
                                    )}
                                    {isTreasurerClub && status === 'APPROVED' && !activeSettlement && (
                                        <button
                                            type="button"
                                            onClick={() => setSettlementTarget(proposal)}
                                            className="rounded-lg bg-cyan-500/15 px-3 py-2 text-sm font-semibold text-cyan-300"
                                        >
                                            Gửi quyết toán
                                        </button>
                                    )}
            ...
            <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Tạo đề xuất ngân sách" size="lg">
                <form onSubmit={createProposal} className="space-y-4">
                    ...
                    <FormField label="Tên đề xuất *">
                        <select
                            value={proposalForm.activityChoice}
                            onChange={(event) =>
                                setProposalForm((current) => ({
                                    ...current,
                                    activityChoice: event.target.value,
                                    customTitle: '',
                                }))
                            }
                            required
                            disabled={!proposalForm.clubId}
                            className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-neutral-900 disabled:bg-neutral-100"
                        >
                            <option value="">Chọn hoạt động hoặc báo cáo sự kiện sắp tới</option>
                            {eligibleFutureReports.map((report) => (
                                <option key={`report-${report.id}`} value={`REPORT:${report.id}`}>
                                    [Sự kiện sắp tới] {report.details?.[0]?.activityName || report.period}
                                </option>
                            ))}
                    ...
                    {proposalForm.activityChoice === 'OTHER' && (
                        <FormField label="Tên đề xuất tự nhập *">
                            <input
                                value={proposalForm.customTitle}
                                onChange={(event) =>
                                    setProposalForm((current) => ({ ...current, customTitle: event.target.value }))
                                }
                                required
                                maxLength={200}
                    ...
                    {proposalForm.clubId && eligibleActivities.length === 0 && eligibleFutureReports.length === 0 && (
                        <p className="text-xs text-neutral-500">
                            Không tìm thấy hoạt động hoặc báo cáo sự kiện sắp tới phù hợp. Chọn Khác để tự nhập tên.
                        </p>
                    )}
                    <FormField label="Mô tả mục đích sử dụng ngân sách *">
                        <textarea
                    ...
                    <FormField label="Số tiền đề xuất (VNĐ) *">
                        <input
                            type="number"
                            min="1"
                            value={proposalForm.requestedAmount}
                            onChange={(event) =>
                                setProposalForm((current) => ({ ...current, requestedAmount: event.target.value }))
                            }
                            required
                            className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-neutral-900"
                        />
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            {busyId === 'create' ? 'Đang gửi...' : 'Gửi đề xuất'}
                        </button>
                    </div>
                </form>
            </Modal>
            ...
            <Modal
                ...
                title={`${reviewStage === 'manager' ? 'Chủ nhiệm' : 'Duyệt cuối'} ${reviewAction === 'approve' ? 'phê duyệt' : 'từ chối'}`}
            >
                <form onSubmit={submitReview} className="space-y-4">
                    <p className="text-sm text-neutral-600">
                        {reviewTarget?.clubName} — {reviewTarget?.title}
                    </p>
                    {reviewStage === 'final' && reviewAction === 'approve' && (
                        <FormField label="Số tiền được duyệt (VNĐ) *">
                            <input
                                type="number"
                                min="1"
                                value={reviewAmount}
                    ...
                    <FormField label={reviewAction === 'reject' ? 'Lý do từ chối *' : 'Nhận xét xét duyệt'}>
                        <textarea
                            value={reviewNote}
            ...
            <Modal isOpen={Boolean(settlementTarget)} onClose={() => setSettlementTarget(null)} title="Gửi quyết toán">
                <form onSubmit={submitSettlement} className="space-y-4">
                    <p className="text-sm text-neutral-600">
                        {settlementTarget?.title} — tối đa {money(settlementTarget?.approvedAmount)}
                    </p>
                    <FormField label="Tổng chi thực tế (VNĐ) *">
                        <input
                            type="number"
                            min="1"
                            max={settlementTarget?.approvedAmount || undefined}
                            value={settlementForm.totalSpent}
                            onChange={(event) =>
                                setSettlementForm((current) => ({ ...current, totalSpent: event.target.value }))
                            }
                            required
                            className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-neutral-900"
                        />
                    </FormField>
                    <FormField label="Đường dẫn hóa đơn *">
                        <input
                            type="url"
                            value={settlementForm.receiptUrl}
                            onChange={(event) =>
                                setSettlementForm((current) => ({ ...current, receiptUrl: event.target.value }))
                            }
                            required
                            pattern="https://.*"
                            placeholder="https://..."
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            type="submit"
                            disabled={busyId === settlementTarget?.id}
                            className="flex-1 rounded-lg bg-cyan-600 px-4 py-3 font-semibold text-white disabled:opacity-50"
                        >
                            Gửi quyết toán
                        </button>
