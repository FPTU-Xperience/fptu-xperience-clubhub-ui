...
// Keep the catch-all category at the end; "Tất cả" is rendered separately as
// the first, fixed filter in DirectoryFilters.
export function orderDirectoryCategories(categories) {
    return [...categories].sort((left, right) => {
        if (left === right) return 0;
        if (left === 'Khác') return 1;
        if (right === 'Khác') return -1;
        return 0;
    });
}
