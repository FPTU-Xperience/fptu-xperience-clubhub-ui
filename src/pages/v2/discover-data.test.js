...
test('directory filter keeps the catch-all category last', () => {
    assert.deepEqual(orderDirectoryCategories(['Học thuật', 'Khác', 'Thể thao']), ['Học thuật', 'Thể thao', 'Khác']);
});
...
test('production V2 modules do not import demo state and demo links stay under /v2/demo', () => {
    const productionFiles = [
        'discovery-page/DiscoveryPage.jsx',
        'all-clubs-page/AllClubsPage.jsx',
        'club-detail-page/ClubDetailPage.jsx',
    ];
    for (const file of productionFiles) {
        const source = readFileSync(new URL(file, import.meta.url), 'utf8');
        assert.doesNotMatch(source, /DemoContext|\.\/model|UI LAB/);
    }
    for (const file of ['DemoApp.jsx', 'Discovery.jsx', 'club-workspace/Workspace.jsx', 'profile-page/Profile.jsx', 'ui.jsx']) {
        const source = readFileSync(new URL(file, import.meta.url), 'utf8');
...
test('production Discover composes the complete demo layout instead of a redesigned page', () => {
    const page = readFileSync(new URL('discovery-page/DiscoveryPage.jsx', import.meta.url), 'utf8');
    ...
    assert.match(page, /MAX_PAGES = 3/);
    assert.match(page, /showRecruiting={false}/);
    assert.match(page, /CLB dành cho bạn/);

    const allClubsPage = readFileSync(new URL('all-clubs-page/AllClubsPage.jsx', import.meta.url), 'utf8');
    ...
    const filters = readFileSync(new URL('../../components/v2/directory-filter/DirectoryFilters.jsx', import.meta.url), 'utf8');
    const card = readFileSync(new URL('../../components/v2/club-card/ClubCard.jsx', import.meta.url), 'utf8');
    ...
    const productionStyles = readFileSync(new URL('discovery-page/DiscoveryPage.scss', import.meta.url), 'utf8');
    assert.match(productionStyles, /\.dx-discover-hero/);
});
