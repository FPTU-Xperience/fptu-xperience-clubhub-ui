...
function getNextActivityDate(activity, now = new Date()) {
    ...
    const scheduleStartValue = Date.UTC(
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        const loadDashboard = async () => {
            ...
            setData({
                ...
                reports:
                    results[1].status === 'fulfilled' && Array.isArray(results[1].value?.items)
    ...
    const cards = [
        {
            label: 'Câu lạc bộ có quyền truy cập',
            value: approvedClubs.length,
            color: 'text-cyan-300',
            href: '/clubs',
            show: hasPermission(PERMISSIONS.VIEW_CLUBS),
        },
        {
        ...
        },
        {
            ...
            href: '/notifications',
            show: hasPermission(PERMISSIONS.VIEW_NOTIFICATIONS),
        },
    ...
    return (
        <div className="space-y-6">
            ...
            {isLoading ? (
                ...
                <>
                    <section
                        className={`grid gap-4 ${cards.length > 2 ? 'grid-cols-2 lg:grid-cols-4' : 'sm:grid-cols-2'}`}
                    >
                        {cards.map((card, index) => (
                            ...
                            >
                                <Link
                                    to={card.href}
                                    className="block rounded-xl border border-slate-800 bg-slate-900/60 p-4 transition hover:border-cyan-500/30"
                                >
                        ...
                        {hasPermission(PERMISSIONS.VIEW_ACTIVITIES) && (
                            <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Link to="/activities" className="text-sm font-semibold text-cyan-300">
                                        Xem tất cả
                                    </Link>
                                </div>
                                {upcomingActivities.length === 0 ? (
                                    <p className="mt-5 rounded-xl border border-dashed border-slate-700 p-8 text-center text-sm text-gray-500">
                                            ...
                                            <div key={activity.id} className="rounded-xl bg-slate-950/50 p-4">
                                                <div className="flex items-start justify-between gap-3">
                                                    ...
                                                    <time className="text-right text-xs text-gray-500">
                                                        {formatActivityDate(nextDate)}
                                                    </time>
                                                </div>
                                                {activity.meetingDays?.length > 0 && (
                                                    <p className="mt-2 text-xs text-amber-300">
                                                        Lịch hằng tuần, giờ Việt Nam
                                                    </p>
                                                )}
                                            </div>
                                        ))}
                        ...
                        {hasPermission(PERMISSIONS.VIEW_NOTIFICATIONS) && (
                            <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Link to="/notifications" className="text-sm font-semibold text-cyan-300">
                                        Xem tất cả
                                    </Link>
                                </div>
                                    ...
                                    <div className="mt-4 space-y-3">
                                        {recentNotifications.map((notification) => (
                                            <Link
                                                key={notification.id}
                                                to="/notifications"
                                                className="block rounded-xl bg-slate-950/50 p-4 transition hover:bg-slate-950/80"
                                            >
                                                <div className="flex items-start gap-3">
                                                    <span
                                                        className={`mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full ${notification.isRead ? 'bg-slate-600' : 'bg-cyan-400'}`}
                                                    />
