...
export default function UsersPage() {
    ...
    const toggleLock = async (target) => {
        ...
        try {
            ...
            setUsers((current) =>
                current.map((item) => (item.id === target.id ? { ...item, isLocked: !target.isLocked } : item)),
            );
    ...
    return (
        <div className="space-y-6">
            ...
            <input
                ...
                placeholder="Tìm theo tên, thư điện tử, tên đăng nhập hoặc vai trò..."
                                    ...
                                    return (
                                        <tr key={target.id} className="hover:bg-white/[0.02]">
                                            <td className="px-5 py-4">
                                                <p className="font-semibold text-white">{target.fullName}</p>
                                            ...
                                            <td className="px-5 py-4">
                                                <span className="rounded-full bg-cyan-500/10 px-2.5 py-1 text-xs font-semibold text-cyan-300">
                                                    {ROLE_LABELS[target.roles[0]] || target.roles[0]}
                                                </span>
                                            </td>
                                            <td className="px-5 py-4">
                                                <span
                                                    className={`text-sm font-semibold ${target.isLocked || !target.isActive ? 'text-rose-300' : 'text-green-500'}`}
                                                >
                                                    {target.isLocked
                                                        ? 'Đã khóa'
                                                        : target.isActive
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <button
                                                        type="button"
                                                        aria-label={`${target.isLocked ? 'Mở khóa' : 'Khóa'} ${target.fullName}`}
                                                        ...
                                                        className={`rounded-lg px-3 py-2 text-xs font-semibold disabled:opacity-35 ${target.isLocked ? 'bg-emerald-500/15 text-green-500' : 'bg-rose-500/15 text-rose-300'}`}
                                                    >
                                                        {target.isLocked ? 'Mở khóa' : 'Khóa'}
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
